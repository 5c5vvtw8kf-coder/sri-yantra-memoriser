...
function Tooltip({ x, y, label, script, seq, isMobile, kana }) {
  ...
  const h = isMobile
    ? ((script === 'devanagari' || script === 'bengali' || script === 'gujarati' || script === 'kannada' || script === 'malayalam') ? 36 : script === 'english' ? 34 : 32)
    : ((script === 'devanagari' || script === 'bengali' || script === 'gujarati' || script === 'kannada' || script === 'malayalam') ? 52 : script === 'english' ? 50 : 48)
  const w = isMobile
    ? measureTooltipWidth(label, fontSize, 14, 50, kana)
...
export default function C4View({
  ...
  // path — fixed font size, no per-sequence C4_TOOLTIP_OFFSET, unzoomed
