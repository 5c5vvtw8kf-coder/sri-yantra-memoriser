...
function Tooltip({ circuitNum, script, uiLang = 'en' }) {
  ...
  const charW    = script === 'devanagari' ? 18 : script === 'bengali' ? 15 : script === 'telugu' ? 21 : script === 'tamil' ? 22 : script === 'english' ? 14.5 : 13.5
...
export default function NavaChakreshvariView({
  ...
  const tooltipCircuit = flash ? null
    : memorise ? (hoveredCircuit ?? ((window.innerWidth < 768 || (navigator.maxTouchPoints > 0 && !window.matchMedia('(pointer: fine)').matches)) ? mobileRevealCircuit : null))
