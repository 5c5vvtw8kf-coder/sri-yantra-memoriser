...
export default function C7View({
  ...
  const handleMemoriseClick = (seq) => {
    ...
    clickTimer.current = setTimeout(() => { clickTimer.current = null; markResult(seq, 'wrong') }, 280)
  }
    ...
    <div className="w-full p-4">
              ...
              <>
                ...
                {c7Deities.map(d => {
                  ...
                  return <polygon key={`past-${d.id}`} points={pts} fill="transparent" stroke="none"
                           ...
                           onContextMenu={e => { e.preventDefault(); setContextMenu({ seq, x: e.clientX, y: e.clientY }) }} />
                })}
                {currentSeq <= 8 && (() => {
      ...
      {!done && (
        <div className="mt-2 text-center">
          ...
          {memorise && currentSeq <= 8 && <p className="text-muted" style={{ fontSize: '10px' }}>hover to reveal · dbl-click = memorised · click = not memorised</p>}
        </div>
      )}
      ...
      {contextMenu && (
        <>
          ...
          <div className="fixed z-50 bg-surface-900 border border-surface-600 rounded-lg shadow-2xl overflow-hidden"
               style={{ left: contextMenu.x, top: contextMenu.y, minWidth: '11rem' }}>
            <button className="w-full text-left px-4 py-2.5 text-sm text-cream hover:bg-surface-700 transition-colors"
