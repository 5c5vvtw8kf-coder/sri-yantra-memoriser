 ...
 * C7View.jsx
 *
 * Circuit 7 — Sarvarōgahara Chakra (8 triangles — Vāgdevīs)
 * Rahasya Yoginī
 *
 * ── Modes ─────────────────────────────────────────────────────────────────────
 *
 * Explore  — all 8 triangles cream to highlight the avarana.
...
const C7_CENTROID_MAP = Object.fromEntries(
  triangleData
    .filter(t => t.circuit === 7)
    .map(t => [t.deitySeq, { x: t.cx, y: t.cy }])
)

const C7_POLYGON_MAP = Object.fromEntries(
  triangleData
    .filter(t => t.circuit === 7)
    .map(t => [t.deitySeq, t.points])
)

const C7_DOT_POSITIONS = Object.fromEntries(
  C7_DEITY_ORDER.map((deitySeq, idx) => [idx + 1, C7_CENTROID_MAP[deitySeq]])
)

function triangleIdForSeq(chantSeq) {
  return `tri-c7-${String(C7_DEITY_ORDER[chantSeq - 1]).padStart(2, '0')}`
}

function trianglePointsForSeq(chantSeq) {
  return C7_POLYGON_MAP[C7_DEITY_ORDER[chantSeq - 1]]
}

const { deities } = data
const deityById = Object.fromEntries(deities.map(d => [d.id, d]))

const c7Deities = deities
  .filter(d => d.sectionId === 'circuit-7' && d.role === 'deity')
  .sort((a, b) => a.sequenceInSection - b.sequenceInSection)
const c7Section = data.sections?.find(s => s.circuitNumber === 7 && s.type === 'circuit') || {}

const c7SvaminiDeity = deities.find(d => d.sectionId === 'circuit-7' && d.role === 'chakraSvamini') ?? null
const c7YoginiDeity = deities.find(d => d.sectionId === 'circuit-7' && d.role === 'yoginiType') ?? null
...
function Tooltip({ x, y, label, script, seq, isMobile, kana }) {
  ...
  const charW = isMobile
    ...
    : (script === 'devanagari' ? 18 : script === 'bengali' ? 11 : script === 'telugu' ? 21 : script === 'tamil' ? 22 : script === 'kannada' ? 20 : script === 'malayalam' ? 23 : script === 'english' ? 14.5 : 13.5)
...
function CompletionPanel({ results, onRestart, onNavigate, tr = k => k, uiLang = 'en' }) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-surface-700 bg-surface-900/80 p-5 text-center">
      <p className="text-gold-400 text-base font-medium mb-1">{Object.values(results).filter(v => v === 'correct').length === 10 ? tr('misc.all_memorised') : tr('spot.round_complete')}</p>
      <p className="text-cream text-sm mb-4">
        <span className="text-red-400">{localNumCP(correct,uiLang)}/{localNumCP(10,uiLang)} {tr('misc.memorised')}</span>
...
export default function C7View({
  script          = 'iast',
  onDeitySelect   = () => {},
  highlightId     = null,
  fillAll         = false,
  ...
  const handleMemoriseClick = (seq) => {
    ...
    if (isMobile && !mobileRevealed) {
      const d   = c7Deities[seq - 1]
      const pos = C7_DOT_POSITIONS[seq]
  ...
  const filledRegions = (() => {
    ...
    if (fillAll) {
      for (let i = 1; i <= 8; i++) fills[triangleIdForSeq(i)] = RED
    } else {
      for (let i = 1; i <= 8; i++) fills[triangleIdForSeq(i)] = ACTIVE_REGION
      ...
      <div className="relative w-full flex-1 min-h-0 md:flex-none rounded-xl overflow-hidden shadow-2xl shadow-black/60 md:[padding-bottom:100%]">
        <div className="absolute inset-0">
          ...
          <svg viewBox={activeViewBox} xmlns="http://www.w3.org/2000/svg"
            ...
            <defs>
              <marker id="c7-cw-arrow" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
            ...
            {!memorise && (
              <>
                {/* Direction arrow — in the black gap between C7 seq-1's right edge
                    (271.257,283.3)→(260,299.47) and C6 deitySeq5's left edge
                    (279.184,283.3)→(272.052,299.47). Gap centre ≈ x:271 at y:291. */}
                <path
                  d="M 270,292 L 275,286"
                  fill="none" stroke="#27ae60" strokeWidth="1.0"
                  markerEnd="url(#c7-cw-arrow)"
            ...
            {memorise && !done && (
              <>
                ...
                {currentSeq <= 8 && (() => {
                  const d   = c7Deities[currentSeq - 1]
                  const pos = C7_DOT_POSITIONS[currentSeq]
